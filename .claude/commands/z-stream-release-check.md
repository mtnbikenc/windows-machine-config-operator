...
2. Interpret and present the output to the user with a clear summary covering:
   ...
   - **Image health**: Flag any branch whose catalog freshness grade is below B (action needed),
     or has Critical or Important CVEs from the catalog Security tab. The "Grade C Date" column
     shows the deadline (for healthy images) or when the image first crossed the threshold
     (for degraded images).
...
| `--cutoff-months N` | Override 18-month support window |
...
- Team PRs have merged since the last release tag (excluding bot bumps and version-bump PRs)
- Catalog freshness grade is below B (C, D, or F)
- Catalog Security tab shows Critical or Important CVEs
